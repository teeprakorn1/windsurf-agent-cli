...
function packageAgent(projectDir, options = {}) {
  ...
  const tmpDir = path.join(os.tmpdir(), `aiyu-multi-agent-publish-${agentName}-${crypto.randomUUID()}`);
...
function copyRecursive(src, dest) {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    ...
    // Path traversal guard — prevent files from escaping the target directory
    const resolvedDest = path.resolve(destPath);
    if (!resolvedDest.startsWith(path.resolve(dest))) {
      console.error("Skipping suspicious path: " + entry.name);
      continue;
    }
...
console.log("Use: aiyu-multi-agent run \"your task here\"");
`;
...
## Use with Aiyu MultiAgent

\`\`\`bash
npx ${pkgName}
npx aiyu-multi-agent run "your task here"
\`\`\`
