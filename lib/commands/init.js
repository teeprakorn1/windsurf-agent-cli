...
async function run(projectDir, options = {}) {
  ...
  if (options.dryRun) {
    console.log(chalk.cyan("[DRY RUN] Would run interactive init...\n"));
    return;
  }
  ...
  try {
    ...
    // Copy core skills from package
    copyCoreSkills(agentDir);
    ...
    if (config.isWindsurfProject(projectDir)) {
...
function copyCoreSkills(agentDir) {
  const pkgSkillsDir = path.resolve(__dirname, "../../.windsurf/skills");
  const coreSkills = ["clean-code", "architecture", "testing-patterns"];
  const destSkillsDir = path.join(agentDir, "skills", "core");

  if (!fs.existsSync(pkgSkillsDir)) return;
