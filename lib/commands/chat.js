...
async function run(options = {}) {
  ...
  while (true) {
    ...
    try {
      ...
      if (entry.quality && !entry.quality.pass) {
        console.log(chalk.yellow(`  [Quality Gate: ${entry.quality.mode}] score=${entry.quality.score}, violations=${entry.quality.violations.map(v => v.message).join("; ")}`));
      }
