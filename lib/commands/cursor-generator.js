...
// Commands that need orchestration template (multi-agent coordination)
const ORCHESTRATION_COMMANDS = new Set([
  "elite-orchestrate", "senior-orchestrate", "junior-orchestrate", "orchestrate",
  "backend-orchestration", "frontend-orchestration", "security-orchestration",
  "protocol-orchestration", "user-orchestration", "elite-tech-leader",
]);

// Commands that activate a single specialist agent
const AGENT_COMMANDS = new Set([
  "backend", "frontend", "database", "security", "go", "react", "nextjs", "angular",
  "sveltekit", "express", "nestjs", "php", "python-api", "html5-css", "vbnet", "delphi",
  "linux", "windows", "docker", "cloud", "network", "staff", "accessibility", "data",
  "data-layer", "business-logic", "fullstack", "iot", "electric", "mechatronic",
  "pneumatic", "plc", "chief-machine", "load-balancer", "math", "package-finder",
]);

// Everything else falls through as utility type

function getCommandType(name) {
  if (ORCHESTRATION_COMMANDS.has(name)) return "orchestration";
...
function parseAgentActivation(body) {
  // Match: 🤖 **Active Agent: `agent-name`** | Skills: `skill1, skill2`
  const m = body.match(/Active Agent:\s*`([^`]+)`\*\*\s*\|\s*Skills:\s*`([^`]+)`/);
  if (m) return { agentName: m[1], skills: m[2] };
  return null;
}

// ── Cursor Output Contract Templates ─────────────────────────────────

function buildOutputContract(agentName, skills) {
  return [
    "",
    ...
    "",
    "```",
    `🤖 **Active Agent: \`${agentName}\`** | Skills: \`${skills}\``,
    "```",
    "",
...
function buildOrchestrationTemplate() {
  return [
    "",
...
function buildAgentTemplate(agentName) {
  return [
    "",
    "## Required Behavior",
    "",
    `1. Read the agent's full instructions from \`.windsurf/agents/${agentName}.md\` (or \`.cursor/rules/agents/${agentName}.mdc\`)`,
...
function buildTemplateBlock(commandName, body) {
  const info = parseAgentActivation(body);
  if (!info) return ""; // no agent activation found — skip injection

  const type = getCommandType(commandName);
  let block = buildOutputContract(info.agentName, info.skills);

  switch (type) {
    case "orchestration":
      block += buildOrchestrationTemplate();
      break;
    case "agent":
      block += buildAgentTemplate(info.agentName);
      break;
    case "utility":
      block += buildUtilityTemplate();
      break;
  }
...
function convertWorkflow(srcPath, destPath) {
  ...
  // Inject Cursor Output Contract + type-specific template
  const templateBlock = buildTemplateBlock(name, body);

  fs.mkdirSync(path.dirname(destPath), { recursive: true });
  fs.writeFileSync(destPath, header + templateBlock + body, "utf-8");
}
