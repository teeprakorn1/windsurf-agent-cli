...
async function runAll() {
  ...
  await test("startTrace creates trace with ID", () => {
    ...
    assert.ok(traceId);
    assert.strictEqual(traceId.length, 16); // 8 bytes hex
  });
  ...
  await test("startSpan creates span within trace", () => {
    ...
    assert.ok(spanId);
    assert.strictEqual(spanId.length, 8); // 4 bytes hex
  });
  ...
  await test("checkReadiness returns component checks", () => {
    const readiness = healthCheck.checkReadiness(process.cwd());
  ...
  await test("getFullHealthReport includes system info", () => {
