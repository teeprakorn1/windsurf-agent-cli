...
async function runAll() {
  ...
  await test("circuit breaker tracks LLM call results", async () => {
    // Reset breaker first
    circuitBreaker.resetBreaker("llm");
    const breakerBefore = circuitBreaker.getBreakerStatus("llm");
    ...
    const breakerAfter = circuitBreaker.getBreakerStatus("llm");
  ...
  await test("health check reports queue status", () => {
    const readiness = healthCheck.checkReadiness(ROOT);
    assert.ok(readiness.checks.queue, "Should have queue check");
    assert.ok(["ok", "healthy", "degraded", "not_available"].includes(readiness.checks.queue.status));
  });

  await test("health check reports circuit breaker status", () => {
    const readiness = healthCheck.checkReadiness(ROOT);
    assert.ok(readiness.checks.circuitBreakers, "Should have circuitBreakers check");
  });

  await test("health check reports LLM provider config", () => {
    const readiness = healthCheck.checkReadiness(ROOT);
    assert.strictEqual(readiness.checks.llmProviders.mock, "available");
    assert.strictEqual(readiness.checks.llmProviders.ollama, "local");
  });
  ...
  await test("full E2E: run → trace → metrics → health", async () => {
    ...
    // 4. Verify health
    const health = healthCheck.checkReadiness(ROOT);
