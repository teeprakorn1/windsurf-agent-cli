...
async function runAgent(params) {
  ...
  if (!agent_name) {
    ...
    if (!defaultAgent) {
      return { error: "No agent_name provided and no default agent found. Specify agent_name or run `aiyu-multi-agent init` first." };
    }
    params.agent_name = defaultAgent;
  }
  if (!input) {
  ...
  try {
    ...
    // Track usage
    usage.trackCommand(projectDir, "run", { via: "mcp" });
