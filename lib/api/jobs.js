...
function mountJobRoutes(app) {
  ...
  app.post("/jobs", async (req, res, next) => {
    try {
      const { agent_name, input, provider, model, max_steps, output_format, no_form, no_quality_gate, strict_quality_gate } = req.body;
      ...
      const jobFn = async () => {
        const result = await agentRuntime.runAgent({
          ...
          noForm: no_form,
          noQualityGate: no_quality_gate,
          strictQualityGate: strict_quality_gate,
        });
