 ...
 *   { type: "chat.complete", sessionId, content, usage, traceId }
 *   { type: "pong" }
...
async function handleRun(ws, msg) {
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      timeoutId = setTimeout(() => reject(new Error(`WS run timed out after ${WS_RUN_TIMEOUT_MS / 1000}s`)), WS_RUN_TIMEOUT_MS);
    });
...
async function handleChatSend(ws, msg) {
  ...
  try {
    ...
    let chatTimeoutId;
    const timeoutPromise = new Promise((_, reject) => {
      chatTimeoutId = setTimeout(() => reject(new Error(`WS chat timed out after ${WS_RUN_TIMEOUT_MS / 1000}s`)), WS_RUN_TIMEOUT_MS);
    });
