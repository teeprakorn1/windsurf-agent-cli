...
function safeWrite(filePath, content, encoding = "utf-8") {
  const safePath = pathTraversal(filePath);
  const dir = path.dirname(safePath);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const tmp = path.join(os.tmpdir(), `windsurf-agent-${Date.now()}-${path.basename(safePath)}`);
