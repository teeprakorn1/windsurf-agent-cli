        ...
        return await new Promise((resolve) => {
          ...
          const req = https.request(url, {
            ...
            res.on("end", () => {
              try {
                const json = JSON.parse(data);
                const web = (json.results && json.results.web) || [];
                const news = (json.results && json.results.news) || [];
                const merged = [...web, ...news].slice(0, maxResults);
                const results = merged.map((r) => ({
                  title: r.title || "",
                  url: r.url || "",
                  snippet: r.description || (Array.isArray(r.snippets) ? (r.snippets[0] || "") : "") || "",
                }));
                resolve({ query, provider, results, count: results.length });
              } catch {
                resolve({ error: "Failed to parse You.com response", query, provider });
              }
            });
          });
          req.on("error", (err) => { resolve({ error: `You.com request failed: ${err.message}`, query, provider }); });
          req.end();
        });
      }

      return { error: `Unknown web search provider: ${provider}. Supported: searxng, serper, tavily, youcom.`, query, provider };
    } catch (err) {
