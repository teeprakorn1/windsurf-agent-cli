...
function install(projectDir, name) {
  ...
  const tmpDir = path.join(require("os").tmpdir(), `aiyu-multi-agent-skill-install-${require("crypto").randomUUID()}`);
  fs.mkdirSync(tmpDir, { recursive: true });

  try {
    // Intentionally uses execFileSync (not sandboxExec) — execFileSync does NOT invoke a shell,
    // so command injection is impossible. packageName is passed as an argv element, not a shell string.
    execFileSync("npm", ["install", packageName, "--prefix", tmpDir, "--no-save", "--no-package-lock", "--ignore-scripts"], {
      cwd: tmpDir,
      encoding: "utf-8",
      stdio: "pipe",
      timeout: 60000,
  ...
  const sourceDir = fs.existsSync(pkgDir) ? pkgDir : (fs.existsSync(altDir) ? altDir : null);
...
function registerSkill(projectDir, skillName) {
  ...
  // Note: configFile is inside cfgDir (.agent/ or .windsurf/). When .windsurf/ is a symlink
  // to .agent/, safeWrite resolves via realpathSync and writes to .agent/ — this is correct.
  // However, if .windsurf/ is manually symlinked outside the project, safeWrite will block
  // the write (pathTraversal guard). In that case, skill registration will fail — this is
  // intentional security behavior.
  if (!cfg.skills.installed.includes(skillName)) {
    cfg.skills.installed.push(skillName);
    guardrails.safeWrite(configFile, YAML.stringify(cfg), "utf-8", projectDir);
  }
}
