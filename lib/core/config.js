...
function initConfigDir(projectDir) {
  const agentDir = getAgentDir(projectDir);
  const windsurfDir = getWindsurfDir(projectDir);
  ...
  if (isWindsurfProject(projectDir) && !fs.existsSync(windsurfDir)) {
    try {
      fs.symlinkSync(agentDir, windsurfDir, "junction");
    } catch {
      // junction may fail on some systems; copy as fallback
...
function saveVersion(projectDir, version) {
  const configDir = getConfigDir(projectDir) || getAgentDir(projectDir);
