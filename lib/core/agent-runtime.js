...
function _cacheKey(input, agentName, provider, model, outputFormat, deterministic, maxSteps) {
  const crypto = require("crypto");
  return crypto.createHash("sha256").update(`${input}|${agentName}|${provider}|${model}|${outputFormat || "text"}|${deterministic || false}|${maxSteps || DEFAULT_MAX_STEPS}`).digest("hex").slice(0, 16);
}
...
async function runAgent(options) {
  ...
  // Cache check
  const cacheKey = _cacheKey(safeInput, agentName, provider, model, options.outputFormat, options.deterministic, overrideMaxSteps);
  if (!noCache) {
...
function createChatSession(options) {
  ...
  return {
    ...
    async send(userInput) {
      ...
      if (agentSpec.outputFormat && agentSpec.outputFormat !== "text") chatLlmOpts.outputFormat = agentSpec.outputFormat;
