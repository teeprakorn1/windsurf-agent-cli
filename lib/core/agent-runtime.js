...
function _cacheKey(input, agentName, provider, model, outputFormat, deterministic) {
  const crypto = require("crypto");
  return crypto.createHash("sha256").update(`${input}|${agentName}|${provider}|${model}|${outputFormat || "text"}|${deterministic || false}`).digest("hex").slice(0, 16);
}
...
async function runAgent(options) {
  ...
  // Cache check
  const cacheKey = _cacheKey(safeInput, agentName, provider, model, options.outputFormat, options.deterministic);
  if (!noCache) {
