...
const TOOL_TIMEOUT_MS = 30000; // 30s per tool call — matches README spec
...
async function runAgent(options) {
  ...
  // Provider failover chain: try next provider when circuit breaker is OPEN
  const FAILOVER_CHAIN = ["openai", "claude", "local", "mock"];
  function getNextProvider(currentProvider) {
    const idx = FAILOVER_CHAIN.indexOf(currentProvider);
    if (idx === -1) return "mock";
    for (let i = idx + 1; i < FAILOVER_CHAIN.length; i++) {
      const next = FAILOVER_CHAIN[i];
      // Check if next provider has credentials configured
      if (next === "openai" && !process.env.OPENAI_API_KEY) continue;
      if (next === "claude" && !process.env.ANTHROPIC_API_KEY) continue;
      if (next === "local") { /* ollama may or may not be running, try anyway */ }
      return next;
    }
    return null; // no more providers
  }
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (state._intervention) {
      ...
      logger.info(`Intervention injected at step ${step + 1}: ${interventionMsg.slice(0, 100)}`);
    }

    // Circuit breaker check with provider failover
    if (!circuitBreaker.canExecute("llm")) {
      // Try failover to next available provider
      const nextProvider = getNextProvider(llmOpts.provider);
      if (nextProvider) {
        logger.warn(`Circuit breaker OPEN for ${llmOpts.provider} — failing over to ${nextProvider}`);
        tracing.addSpanEvent(traceId, stepSpanId, "provider_failover", { from: llmOpts.provider, to: nextProvider });
        llmOpts.provider = nextProvider;
        // Reset circuit breaker for the new provider attempt
        if (!circuitBreaker.canExecute("llm")) {
          // New provider also has breaker open — try further
          const furtherProvider = getNextProvider(nextProvider);
          if (furtherProvider) {
            llmOpts.provider = furtherProvider;
            logger.warn(`Failover chain: ${furtherProvider}`);
          } else {
            ...
            state.error = `All LLM providers unavailable (circuit breaker OPEN). Retry after ${retrySeconds}s`;
            tracing.endSpan(traceId, stepSpanId, { error: state.error, errorType: "circuit_breaker_exhausted" });
    ...
    try {
      ...
      // On failure, try failover to next provider within same step
      const nextProvider = getNextProvider(llmOpts.provider);
      if (nextProvider) {
        logger.warn(`LLM call failed for ${llmOpts.provider}: ${err.message} — failing over to ${nextProvider}`);
        tracing.addSpanEvent(traceId, stepSpanId, "provider_failover_on_error", { from: llmOpts.provider, to: nextProvider, error: err.message });
        llmOpts.provider = nextProvider;
        try {
          response = await llmProviders.callLLM(messages, llmOpts);
          circuitBreaker.recordSuccess("llm");
        } catch (failoverErr) {
          circuitBreaker.recordFailure("llm", failoverErr);
          state.status = "error";
          state.error = `All LLM providers failed at step ${step + 1}. Primary: ${err.message}. Failover: ${failoverErr.message}`;
          tracing.endSpan(traceId, stepSpanId, { error: state.error, errorType: "llm_failover_exhausted" });
          break;
        }
      } else {
        state.status = "error";
  ...
  while ((tcMatch = tcRegex.exec(content)) !== null) {
    ...
    for (; endIdx < content.length && depth > 0; endIdx++) {
      ...
      if (inStr) {
        if (escaped) {
          escaped = false;
          // Don't continue — process this char normally (it was the char after the escape)
        } else if (ch === "\\") {
          escaped = true;
          continue;
        } else if (ch === strCh) {
    ...
    async send(userInput) {
      ...
      for (let step = 0; step < maxChatSteps; step++) {
        ...
        if (!circuitBreaker.canExecute("llm")) {
          ...
          chatSteps.push({ step: step + 1, thought: finalContent, toolCalls: [], duration_ms: Date.now() - stepStart, error: finalContent });
          break;
        }
