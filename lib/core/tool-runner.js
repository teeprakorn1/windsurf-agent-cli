...
const NET_TOOLS = ["search.grep"]; // could involve network paths
...
(async () => {
  try {
    ...
    if (resultStr.length > MAX_SIZE) {
      ...
      if (truncated.content) truncated.content = truncated.content.slice(0, MAX_SIZE) + "...[truncated]";
      if (truncated.stdout) truncated.stdout = truncated.stdout.slice(0, MAX_SIZE) + "...[truncated]";
