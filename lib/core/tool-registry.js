          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              for (let i = 0; i < lines.length; i++) {
                regex.lastIndex = 0; // Reset before each test to prevent stateful 'g' flag issues
                try {
  ...
  "fs.glob": async (args) => {
    ...
    try {
      ...
      const globToRegex = (globPattern) => {
        // Expand {a,b,c} brace alternation — escape each alternative individually before joining
        const braceGroups = [];
        ...
        // Restore brace alternation groups
...
function truncateResult(result) {
  ...
  // Truncate largest string fields on the original object to avoid double serialize/parse
