...
const BUILTIN_TOOLS = {
  ...
  "search.grep": async (args) => {
    ...
    try {
      // Use Node.js for cross-platform compatibility (no grep dependency)
      const fs = require("fs");
    ...
    try {
      ...
      const globToRegex = (glob) => {
        let regex = glob
          ...
          .replace(/\*/g, "[^/]*") // single star = any chars except /
  ...
  "shell.exec": async (args) => {
    ...
    try {
      ...
      // Detect command substitution patterns (dangerous even in args with execFileSync if passed to shell)
      // Note: > | ; && || etc. are NOT dangerous with execFileSync (no shell), only command substitution is
      ...
      // Block flags that enable arbitrary code execution (node -e, python3 -c, etc.)
      for (const arg of cmdArgs) {
        if (guardrails.BLOCKED_FLAGS && guardrails.BLOCKED_FLAGS.includes(arg)) {
          return { error: `Flag "${arg}" not allowed — arbitrary code execution blocked`, exitCode: 1 };
        }
      }
      ...
      // Use execFileSync (no shell injection possible)
...
function truncateResult(result) {
  ...
  if (typeof truncated.content === "string" && truncated.content.length > MAX_RESULT_SIZE) {
    truncated.content = truncated.content.slice(0, MAX_RESULT_SIZE) + "...[truncated]";
  }
  if (typeof truncated.stdout === "string" && truncated.stdout.length > MAX_RESULT_SIZE) {
    truncated.stdout = truncated.stdout.slice(0, MAX_RESULT_SIZE) + "...[truncated]";
  }
  if (Array.isArray(truncated.matches) && JSON.stringify(truncated.matches).length > MAX_RESULT_SIZE) {
    truncated.matches = truncated.matches.slice(0, 50);
  }
  if (Array.isArray(truncated.files) && JSON.stringify(truncated.files).length > MAX_RESULT_SIZE) {
