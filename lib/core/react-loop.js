  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (totalChars > MAX_CONTEXT_CHARS) {
      ...
      while (messages.reduce((sum, m) => sum + (m.content?.length || 0), 0) > MAX_CONTEXT_CHARS && messages.length > 3) {
        // Find the first non-system message pair to drop
        // Drop assistant[1] + user[2] if they form a pair, otherwise just drop [1]
        const first = messages[1];
        const second = messages[2];
        if (first && second && first.role === "assistant" && second.role === "user") {
          messages.splice(1, 2);
        } else if (first) {
          messages.splice(1, 1);
        } else {
