...
function _syncFlushBuffer() {
  // Sync fallback for process "exit" event (no async allowed)
  if (_bufferedUsage && _bufferProjectDir) {
    try {
      const filePath = getUsageFilePath(_bufferProjectDir);
      if (filePath) {
        const dir = path.dirname(filePath);
        if (!fs.existsSync(dir)) return;
        // Cleanup stale .tmp from previous crash before writing new one
        const staleTmp = filePath + ".tmp";
        try { fs.unlinkSync(staleTmp); } catch { /* not present — ok */ }
        // Atomic write: temp file + rename to prevent truncated file on crash
        const tmpPath = filePath + ".tmp";
        fs.writeFileSync(tmpPath, JSON.stringify(_bufferedUsage, null, 2), "utf-8");
...
function trackCommand(projectDir, command, meta = {}) {
  ...
  if (!_exitHandlerRegistered) {
    ...
    // beforeExit allows async work (unlike 'exit' which is sync-only)
    process.on("beforeExit", () => { _flushBuffer(); });
    // Fallback for forced termination: sync write (covers SIGKILL-like scenarios)
    process.on("exit", _syncFlushBuffer);
  }
