...
async function checkReadiness(projectDir) {
  ...
  const hasAnyProvider = process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY || process.env.OLLAMA_HOST;
  checks.llmProviders = {
    status: hasAnyProvider ? "ok" : "not_configured",
    message: hasAnyProvider ? undefined : "No LLM provider configured. Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or OLLAMA_HOST.",
