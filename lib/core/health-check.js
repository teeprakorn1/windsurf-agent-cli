...
async function checkReadiness(projectDir) {
  ...
  try {
    ...
    // Reuse a single agent per transport type — destroy after use to prevent GC pressure
    ...
    // Destroy agent after check to free sockets — new one created on next check
    _ollamaAgent.destroy();
    _ollamaAgent = null;
