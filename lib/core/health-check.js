...
async function checkReadiness(projectDir) {
  ...
  try {
    ...
    // Use a fresh agent with keepAlive:false to prevent socket leak in long-running servers
    const healthCheckAgent = new transport.Agent({ keepAlive: false, timeout: 3000 });
    const checkResult = await new Promise((resolve, reject) => {
      const req = transport.request({ hostname: ollamaUrl.hostname, port: ollamaUrl.port, path: "/api/tags", method: "GET", timeout: 2000, agent: healthCheckAgent }, (res) => {
  ...
  checks.llmProviders = {
    ...
    message: hasAnyProvider ? undefined : "No LLM API keys configured. Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or OLLAMA_HOST.",
    openai: process.env.OPENAI_API_KEY ? "configured" : "not_configured",
    claude: process.env.ANTHROPIC_API_KEY ? "configured" : "not_configured",
    ollama: ollamaStatus,
    mock: process.env.AIYU_ENABLE_MOCK === "1" ? "enabled" : "disabled",
  };
