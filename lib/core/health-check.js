...
async function checkReadiness(projectDir) {
  ...
  const cliConfig = config.loadCliEngineConfig(projectDir);
  const cliEngines = cliScanner.listAvailableEngines({ config: cliConfig });
  const cliReport = {};
  for (const engine of cliEngines) {
    cliReport[engine.name] = engine.available ? "available" : "not_found";
  }

  const hasAnyProvider = process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY || process.env.GROQ_API_KEY || process.env.OLLAMA_HOST || cliEngines.length > 0;
  checks.llmProviders = {
    status: hasAnyProvider ? "ok" : "not_configured",
    message: hasAnyProvider ? undefined : "No LLM provider configured. Set OPENAI_API_KEY, ANTHROPIC_API_KEY, GROQ_API_KEY, or OLLAMA_HOST, or add a CLI engine to PATH.",
