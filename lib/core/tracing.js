...
async function _flushWriteQueue() {
  ...
  try {
    ...
    // If more items queued while we were writing, flush again
    if (_writeQueue.length > 0) _flushWriteQueue();
  }
}
...
function getTraceMetrics() {
  ...
  return {
    ...
    p95DurationMs: completed.length > 0
      ? completed.map(t => t.durationMs || 0).sort((a, b) => a - b)[Math.min(Math.floor(completed.length * 0.95), completed.length - 1)] || 0
      : 0,
