...
class RequestQueue extends EventEmitter {
  ...
  enqueue(fn, options = {}) {
    ...
    const timeout = options.timeout || this.jobTimeoutMs;
  ...
  waitFor(id) {
    return new Promise((resolve, reject) => {
      ...
      const checkInterval = setInterval(() => {
        ...
        if (!j) { clearInterval(checkInterval); clearTimeout(timeoutTimer); if (!settled) { settled = true; reject(new Error(`Job ${id} lost`)); } return; }
        if (j.status === "completed") {
          clearInterval(checkInterval); clearTimeout(timeoutTimer);
          if (!settled) { settled = true; resolve(j.result); }
        } else if (j.status === "failed") {
          clearInterval(checkInterval); clearTimeout(timeoutTimer);
          if (!settled) { settled = true; reject(new Error(j.error)); }
        }
      }, 50);

      const timeoutTimer = setTimeout(() => {
        clearInterval(checkInterval);
        if (!settled) { settled = true; reject(new Error(`Wait timeout for job ${id}`)); }
      }, (job.timeout || this.jobTimeoutMs) + 5000);
    });
  }
