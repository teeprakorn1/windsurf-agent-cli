...
// resolveProvider() is defined in failover.js — single canonical source
// This module is called by failover.js, so we avoid circular dependency by
// having callLLM() accept provider explicitly (always passed by failover chain)
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      switch (provider) {
        ...
        default:
          throw new Error(`Unknown LLM provider: "${provider}". Available: openai, claude, local, mock`);
      }
    } catch (err) {
      lastError = err;
