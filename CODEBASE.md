# CODEBASE.md — Aiyu MultiAgent V2.5.1
...
### V2 Modules
- `lib/core/agent-runtime.js` — 🔥 ReAct loop, chat session (sliding window + char-based context limit MAX_CONTEXT_CHARS=200000, step records, outputFormat enforcement), agent loader, **per-provider circuit breaker** (`llm:openai`, `llm:claude` etc.) with `callLLMWithFailover()` chain + `isAnyLlmAvailable()` check, distributed tracing (traceId in state), MAX_ALLOWED_STEPS=50 hard cap, balanced-depth parseToolCalls (with proper `let match` declaration, escaped flag fix — no char swallow after escape), cache key with agentInstructionsHash + resolved outputFormat/deterministic/maxSteps (not raw options), cache deep clone only for object/array + **Object.freeze() fallback** for circular refs, **crypto top-level require**
- `lib/core/tool-registry.js` — 🔥 Namespaced tools (fs.read/shell.exec with offset/limit support), schemas, validation, parseCommandArgs (escape sequences), cross-platform fs.glob/search.grep (maxDepth, maxFileSize, maxFiles limits, fs require fix, glob fallback depth limit, wildcard-before-escape regex fix, node_modules/.git skip in fallback, async walk with setImmediate yield every 50 files, **search.grep for-loop with lastIndex reset** to prevent g-flag match skipping), truncateResult reuses stringify (no double parse), executeToolIsolated passes cwd in fork. fs.edit enforces unique old_string (rejects multiple occurrences). shell.exec rejects path-prefixed commands (./node bypass) and passes bare command name to sandboxExec. **fs.glob brace alternation escapes metacharacters in each alternative individually**
- `lib/core/llm-providers.js` — 🔥 OpenAI, Claude (tool_use), Ollama (tools), Mock (respects outputFormat), retry/backoff (**fixed off-by-one: exactly MAX_RETRIES attempts**), 1MB response size limit, default temperature 0.7 for all providers
...
- `lib/core/usage.js` — Usage statistics + deployment tracking + agentRuns counter + Prometheus metrics export (formatPrometheusMetrics) + getMetrics + safeWrite with projectDir (not cfgDir) for correct pathTraversal scope + **process.on("exit") flush** to prevent data loss
...
- `lib/core/tracing.js` — 🔥 Distributed tracing (trace+span IDs, OpenTelemetry export), trace storage, metrics (avg/p95 duration with **Math.min p95 index clamp**). Sync file append (fs.appendFileSync) to prevent race condition during rotation
