...
> **Guide to creating and using Skills in the Aiyu Agent Kit**

---

## 📋 Overview

While Aiyu's base models (like Gemini) are powerful generalists, they don't know your specific project context or your team's standards. Loading every rule or tool into the agent's context window leads to "tool bloat," higher costs, latency, and confusion.

**Aiyu Agent Skills** solve this through **Progressive Disclosure**. A Skill is a package of specialized knowledge that remains dormant until needed. This information is only loaded into the agent's context when your specific request matches the skill's description.
...
| ------------- | --------------------------------- | ------------------------------------ |
| **Workspace** | `<workspace-root>/.windsurf/skills/` | Available only in a specific project |
...
```bash
mkdir -p .windsurf/skills/code-review
```
...
```bash
mkdir -p .windsurf/skills/license-header-adder/resources
```

### Step 2: Create the template file

**`.windsurf/skills/license-header-adder/resources/HEADER.txt`**:
...
**`.windsurf/skills/license-header-adder/SKILL.md`**:
