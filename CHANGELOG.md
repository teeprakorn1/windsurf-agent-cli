...
- **Custom / Proxy Endpoint Support** — `OPENAI_BASE_URL` and `ANTHROPIC_BASE_URL` environment variables redirect the `openai` and `claude` providers to any OpenAI-compatible or Anthropic-compatible API endpoint. Enables usage with proxy services (MaxPlus AI, OpenRouter), Chinese providers (GLM-5.1, Kimi-K2.6, DeepSeek), and self-hosted gateways without adding new provider types. Backward compatible: omit the variables and the providers default to the official APIs (`lib/core/llm-providers.js`).
- **`/config` chat command** — Manage API keys, base URLs, and default provider/model from within any chat session. Settings persist in `~/.aiyu/config.json`. Sub-commands: `/config show`, `/config set <provider> <field> <value>`, `/config default <provider> [model]`, `/config reset`. Priority: env var > user config > agent spec > hardcoded default (`lib/core/user-config.js`, `lib/commands/chat.js`).
- **Slash command menu (keypress-based, interactive, with live filtering)** — Typing `/` instantly shows the command menu without pressing Enter, matching Hermes Agent behavior. Features: **live filtering** — typing after `/` filters the list (e.g. `/c` shows only `/config`); **arrow-key navigation** (Up/Down with yellow highlight); **ESC to cancel** and return to empty prompt; **Backspace to remove filter chars** (or exit menu if filter is empty); **Enter to select** a command. Uses `readline.emitKeypressEvents` with ANSI escape codes for real-time redraw. Commands: `/new`, `/save`, `/retry`, `/undo`, `/model`, `/config`, `/help`. Falls back to line-based input on non-TTY terminals (`lib/commands/chat.js`).
- **ESC cancellation** — Two-phase cancel behavior: (1) **While typing** (before Enter): ESC clears the entire input buffer, allowing the user to start over; (2) **While AI is processing** (after Enter): ESC marks the in-flight request as cancelled, discards the response when it arrives, and immediately returns to the prompt. Also supports Ctrl+C as an alternative cancel key (`lib/commands/chat.js`).
- **Input history (Up/Down arrows)** — Pressing Up arrow in the input prompt recalls previously sent messages (most recent first). Down arrow moves forward in history. ESC clears input but history is preserved. Stores up to 100 unique messages per session. Slash commands are excluded from history (`lib/commands/chat.js`).
- **`/model` chat command** — Switch models within the current provider only. `/model` lists available models for the active provider with a `*` marker on the current one. `/model <name>` switches to that model (creates a new chat session preserving agent). If the requested model does not belong to the current provider, suggests using `/config default <provider> <model>` to change providers (`lib/commands/chat.js`).
- **Real-time AI process visibility** — While the agent is processing, each step is displayed live in the terminal (IDE-like experience). Shows: `[Step N] Thinking...` with a preview of the agent's thought, `[Step N] Calling tool: <name>` with args preview, `[Step N] Tool <name> done:` with result preview, `[Step N] Tool <name> error:` on failure. After completion, shows `[Done] N step(s), X.Xs, N tokens` summary. Implemented via `onStep` callback in `createChatSession` (`lib/core/chat-session.js`, `lib/commands/chat.js`).
